import tkinter as tk                       #kept for Canvas widget (no CustomTkinter equivalent)
from tkinter import filedialog             #tool that opens the "Choose a File" pop-up window so the user can select a photo
import customtkinter as ctk                #modern dark-mode widget toolkit
...
class DefocusAppUI:
    def __init__(self, root, processor):                     #when the window is created, it takes root and processor
        ...
        self.root.title(f"{Config.PROJECT_NAME}")     #sets the text at the very top of the window bar
        ...
        self.secondary_focal_point = None                     #Split Diopter: second focus point (None = single-focus mode)

        self.setup_ui()                                      #calls the next function to start drawing the buttons

    def setup_ui(self):
        # control Bar
        toolbar = ctk.CTkFrame(self.root, fg_color=Config.PANEL_COLOR)    #horizontal bar at the top for controls
        toolbar.pack(side=tk.TOP, fill=tk.X, ipady=10) #sticks this bar to the top of the window and stretches it across the X-axis

        ctk.CTkButton(toolbar, text="UPLOAD IMAGE", command=self.load_image, font=("Arial", 13, "bold")).pack(side=tk.LEFT, padx=20) #Creates the Upload button
        
        # sliders
        # creates the two sliding bars for intensity and quality
        ctk.CTkLabel(toolbar, text="Blur Intensity", font=("Arial", 12), text_color="white").pack(side=tk.LEFT, padx=(20, 5))
        self.blur_slider = ctk.CTkSlider(toolbar, from_=5, to=80, number_of_steps=75, command=self._on_blur_change)
        self.blur_slider.set(30)   #sets default to 30
        self.blur_slider.pack(side=tk.LEFT, padx=(0, 5))
        self.blur_val_label = ctk.CTkLabel(toolbar, text="30", font=("Arial", 12, "bold"), text_color=Config.ACCENT_COLOR, width=30)
        self.blur_val_label.pack(side=tk.LEFT, padx=(0, 20))

        ctk.CTkLabel(toolbar, text="Render Quality (Layers)", font=("Arial", 12), text_color="white").pack(side=tk.LEFT, padx=(20, 5))
        self.layer_slider = ctk.CTkSlider(toolbar, from_=3, to=30, number_of_steps=27, width=200, command=self._on_layer_change)
        self.layer_slider.set(12)
        self.layer_slider.pack(side=tk.LEFT, padx=(0, 5))
        self.layer_val_label = ctk.CTkLabel(toolbar, text="12", font=("Arial", 12, "bold"), text_color=Config.ACCENT_COLOR, width=30)
        self.layer_val_label.pack(side=tk.LEFT, padx=(0, 20))

        ctk.CTkButton(toolbar, text="DOWNLOAD IMAGE", command=self.save_image, font=("Arial", 13, "bold"), fg_color=Config.SUCCESS_COLOR, hover_color="#27ae60").pack(side=tk.RIGHT, padx=20) #Save button on the right side of toolbar

        # 3 panels
        workspace = ctk.CTkFrame(self.root, fg_color=Config.BG_COLOR) #creates a giant empty space below the toolbar to hold the images
        ...
        self.canvas_orig.bind("<Shift-Button-1>", self.set_secondary_focus) #Shift+click sets the second focal plane for Split Diopter mode
        self.canvas_orig.bind("<Configure>", self._on_resize) #re-renders images when panels are resized

        # panel 2 Depth View
        frame_depth = ctk.CTkFrame(workspace, fg_color="black", corner_radius=0)
        ...
        self.lbl_depth = tk.Label(frame_depth, bg="black", highlightthickness=1, highlightbackground="white")
        ...
        # panel 3 Output View
        frame_final = ctk.CTkFrame(workspace, fg_color="black", corner_radius=0)
        ...
        self.lbl_final = tk.Label(frame_final, bg="black", highlightthickness=1, highlightbackground="white")
    ...
    def handle_click(self, event):
        ...
        if 0 <= tx < iw and 0 <= ty < ih:
            self.focal_point = self.depth_map[ty, tx]  #sets this as the new focus 
            self.secondary_focal_point = None           #clears Split Diopter, back to single-focus
            print(f"System: Focusing on depth {self.focal_point} at pixel ({tx}, {ty}) [Single Focus]")
            self.reprocess() #re-renders the blur with this new focus

    def set_secondary_focus(self, event):
        """Shift+Click: Sets a second focal plane for Split Diopter mode."""
        if self.depth_map is None: return
        ...
        if 0 <= tx < iw and 0 <= ty < ih:
            self.secondary_focal_point = self.depth_map[ty, tx]
            print(f"System: Split Diopter ON — Secondary focus at depth {self.secondary_focal_point} at pixel ({tx}, {ty})")
            self.reprocess()

    def _on_blur_change(self, value):
        """Updates the blur value label and re-renders."""
        self.blur_val_label.configure(text=str(int(value)))
        self.reprocess()

    def _on_layer_change(self, value):
        """Updates the layer value label and re-renders."""
        self.layer_val_label.configure(text=str(int(value)))
        self.reprocess()
    ...
    def reprocess(self): #tells the processor to run the complex blur math using the new focus, the blur slider value, and the layer slider value
        ...
        # Build focal_depth: single value or tuple for Split Diopter
        if self.secondary_focal_point is not None:
            focal = (self.focal_point, self.secondary_focal_point)
        else:
            focal = self.focal_point
        
        self.processed_img = self.processor.apply_defocus(
            self.original_img, self.depth_map, focal, 
            int(self.blur_slider.get()), int(self.layer_slider.get())
        )
