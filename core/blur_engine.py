...
class BlurEngine:
    ...
    def apply_layered_defocus(self, original_img, depth_map, focal_depth, max_blur_radius=25, num_layers=15, dof_thickness=20):
        ...
        # support Split Diopter (multi-focus): focal_depth can be a single value or a tuple of two
        if isinstance(focal_depth, (list, tuple)):
            focal_depth_1 = float(focal_depth[0])
            focal_depth_2 = float(focal_depth[1])
        else:
            focal_depth_1 = float(focal_depth)
            focal_depth_2 = None  #single-focus mode
        ...
        for layer_idx in range(1, len(bins) + 1): #This starts a loop. We are going 1 layer at a time
            ...
            # Split Diopter: blur based on minimum distance to either focal plane
            distance = abs(layer_depth - focal_depth_1)
            if focal_depth_2 is not None:
                distance = min(distance, abs(layer_depth - focal_depth_2))
